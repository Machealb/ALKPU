...
def make_v0_posars(config):
    ...
    for i in range(0, nums):
        lattices.append(round(lattice + gap*i, 2))

    for lat in lattices:
        with open(cell_poscar_path, 'r') as rf:
            baselines = rf.readlines()
        newlines = change_poscar_lattice(baselines, lat)
        save_dir = os.path.join(work_dir, '{}/{}'.format(type, lat))
        if os.path.exists(save_dir) is False:
            os.makedirs(save_dir)
        with open(os.path.join(save_dir, "atom.config"), 'w') as wf:
...
def tmp_convert_scf2movement(config):
    work_dir = config["work_dir"]
    type = config["type"]
    work_dir = os.path.join(work_dir, type)
    os.chdir(work_dir)
...
def tmp_test():
    epochs = 30
    file_path = "/share/home/wuxingxing/al_dir/cu_70/final/phy/v0/kpu/tmp_line.txt"
    for epoch in range(0, 31, 3):
        line1 = "MKL_THREADING_LAYER=GNU horovodrun -np 1 python /share/home/wuxingxing/codespace/mutli_mlff/src/dp_main_kpu.py --hvd --opt LKF --epochs {} -r -c /share/home/wuxingxing/al_dir/cu_70/final/final_train/train.yaml -s /share/home/wuxingxing/al_dir/cu_70/final/final_train -b 1\n\n".format(epoch)
        line2 = "MKL_THREADING_LAYER=GNU horovodrun -np 1 python /share/home/wuxingxing/codespace/mutli_mlff/src/dp_kpu.py --hvd --opt LKF --epochs {} -r -b 1 -e --eval_name v0_epoch_{} -c /share/home/wuxingxing/al_dir/cu_70/final/phy/v0/kpu/valid.yaml -s /share/home/wuxingxing/al_dir/cu_70/final/final_train\n\n".format(epoch,epoch)
        write_to_file(file_path, line1)
        write_to_file(file_path, line2)
